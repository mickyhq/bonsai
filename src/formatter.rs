...
pub fn format_repository_context_xml(
    ...
    if options.project_map_only {
        ...
        push_project_map_xml(&mut output, files, options.include_file_hashes);
        return output;
    }
...
pub fn format_repository_context_json(
    ...
    if options.project_map_only {
        let mut output = String::new();
        push_project_map_json(&mut output, files, 0, options.include_file_hashes);
        output.push('\n');
        return output;
    }
    ...
    output.push_str(",\n  \"project_map\": ");
    push_project_map_json(&mut output, files, 2, options.include_file_hashes);
...
fn push_project_map_xml(output: &mut String, files: &[ProcessedFile], include_hash: bool) {
